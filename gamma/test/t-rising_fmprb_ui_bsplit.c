...
int main()
{
    ...
    for (iter = 0; iter < 1000; iter++)
    {
        ...
        fmprb_randtest(b, state, 1 + n_randint(state, 4000), 10);
        n = n_randint(state, 40);

        fmprb_get_rand_fmpq(x, state, a, 1 + n_randint(state, 1000));

        gamma_rising_fmprb_ui_bsplit(b, a, n, 2 + n_randint(state, 4000));
    ...
    for (iter = 0; iter < 1000; iter++)
    {
        ...
        gamma_rising_fmprb_ui_bsplit(x, x, n, prec);
