...
int main()
{
    ...
    for (iter = 0; iter < 10000; iter++)
    {
        ...
        fmpcb_set_fmpq(x, X, rbits2);
        fmpcb_poly_evaluate_horner(y, f, x, rbits3);

        if (!fmpcb_contains_fmpq(y, Y))
        {
            printf("FAIL\n\n");
        ...
        /* aliasing */
        fmpcb_poly_evaluate_horner(x, f, x, rbits3);
        if (!fmpcb_contains_fmpq(x, Y))
        {
