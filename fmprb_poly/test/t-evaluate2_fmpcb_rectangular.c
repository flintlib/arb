...
int main()
{
    ...
    printf("evaluate2_fmpcb_rectangular....");
    fflush(stdout);
    ...
    for (iter = 0; iter < 10000; iter++)
    {
        ...
        fmprb_poly_evaluate2_fmpcb_rectangular(y1, z1, f, x, 2 + n_randint(state, 1000));

        fmprb_poly_evaluate_fmpcb_horner(y2, f, x, 2 + n_randint(state, 1000));
        fmprb_poly_evaluate_fmpcb_horner(z2, g, x, 2 + n_randint(state, 1000));
