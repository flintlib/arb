...
int main()
{
    ...
    for (iter = 0; iter < 100000; iter++)
    {
        ...
        {
            case 0:
                mpfr_sub(Z, X, Y, MPFR_RNDZ);
                fmpr_sub(z, x, y, bits, FMPR_RND_DOWN);
                break;
            case 1:
                mpfr_sub(Z, X, Y, MPFR_RNDA);
                fmpr_sub(z, x, y, bits, FMPR_RND_UP);
                break;
            case 2:
                mpfr_sub(Z, X, Y, MPFR_RNDD);
                fmpr_sub(z, x, y, bits, FMPR_RND_FLOOR);
                break;
            case 3:
                mpfr_sub(Z, X, Y, MPFR_RNDU);
                fmpr_sub(z, x, y, bits, FMPR_RND_CEIL);
                break;
        }
