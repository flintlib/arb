...
int main()
{
    ...
    for (iter = 0; iter < 100000; iter++)
    {
        ...
        {
            case 0:
                mpfr_add(Z, X, Y, MPFR_RNDZ);
                fmpr_add(z, x, y, bits, FMPR_RND_DOWN);
                break;
            case 1:
                mpfr_add(Z, X, Y, MPFR_RNDA);
                fmpr_add(z, x, y, bits, FMPR_RND_UP);
                break;
            case 2:
                mpfr_add(Z, X, Y, MPFR_RNDD);
                fmpr_add(z, x, y, bits, FMPR_RND_FLOOR);
                break;
            case 3:
                mpfr_add(Z, X, Y, MPFR_RNDU);
                fmpr_add(z, x, y, bits, FMPR_RND_CEIL);
                break;
        }
